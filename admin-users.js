document.addEventListener("DOMContentLoaded", () => {
      ...
      userTableBody.innerHTML = `
        <tr>
          <td colspan="5" class="table-message" style="color: var(--danger);">
            ...
            Error loading user database. Please try again.
          </td>
        </tr>
      `;
    }
          ...
          <td>
            <div class="action-group">
              ...
              ${isSelf ? `<span class="self-label">(You)</span>` : `
                <button class="btn" onclick="toggleRole('${user.id}', '${nextRole}')" title="${role === "admin" ? "Demote to Student" : "Promote to Admin"}">
                  <i class="fas ${role === "admin" ? "fa-user-minus" : "fa-user-plus"}"></i>
                  ${role === "admin" ? "Demote" : "Promote"}
                </button>

                <button class="btn warn" onclick="toggleStatus('${user.id}', '${isSuspended ? "active" : "suspended"}')" title="${isSuspended ? "Activate Account" : "Suspend Account"}">
                  <i class="fas ${isSuspended ? "fa-user-check" : "fa-user-lock"}"></i>
                  ${isSuspended ? "Activate" : "Suspend"}
                </button>

                <button class="btn danger" onclick="deleteUser('${user.id}', '${escapeHtml(user.name)}')" title="Permanently Delete User">
  ...
  window.toggleRole = async function(userId, newRole) {
    if (!confirm(`Are you sure you want to change this account's role to ${newRole.toUpperCase()}?`)) return;
    await executeAdminAction("update-role", { userId, role: newRole });
  };

  window.toggleStatus = async function(userId, newStatus) {
    if (!confirm(`Are you sure you want to set this account status to ${newStatus.toUpperCase()}?`)) return;
    await executeAdminAction("update-status", { userId, status: newStatus });
  };

  window.deleteUser = async function(userId, userName) {
    if (!confirm(`PERMANENT ACTION: Are you sure you want to delete user "${userName}"? This action cannot be undone.`)) return;
